...
export default class Buttons extends Component {
  ...
  render(){
    return (
        ...
        <label className="current-calc-label">&nbsp; {this.props.currentCalc}</label>
        <label className="result-label">&nbsp; {this.props.screenValue}</label>
        {
          ...
          return <button key={number} name={number} onClick={(event) => this.props.addToScreenValue(event)}>{number}</button>;
            ...
            <button name="plus" onClick={this.props.plus}>+</button>
            <button name="equals" onClick={this.props.equals}>=</button>
